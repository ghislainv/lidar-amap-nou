...
for (i in 1:nsites) {
  ...
  if (!file.exists(ofile) & site != "dzumac-2") {
    ...
    if (site == "grand-lac") {
      sf_files_union <- st_set_crs(sf_files_union, "epsg:32758")
    }
    # Compute centroid and area (in ha)
    cntrd <- sf::st_centroid(sf_files_union)
    cntrd_coord <- sf::st_coordinates(cntrd)
    area <- sf::st_area(sf_files_union)
    # Create data-frame and spatial feature collection
